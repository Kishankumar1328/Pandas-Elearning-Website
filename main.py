...
#Sidebar Start
def cs_sidebar():
    st.sidebar.markdown('''[<img src='data:image/png;base64,{}' class='img-fluid' width=300 height=120>](https://numpy.org/doc/stable/user/whatisnumpy.html)'''.format(img_to_bytes("side.jpg")), unsafe_allow_html=True)
    st.sidebar.title('Pandas cheat sheet')
    st.sidebar.markdown('''<small>Developed by <b>Kishan Kumar Suresh Kumar<b></small>''', unsafe_allow_html=True)
...
#Data MANIPULATION sTARTS
    ...
    st.subheader("Joining of Two Datasets")
    st.code("df.join(df2)")
    st.write("Join DataFrames based on index or columns")
    st.code("pd.merge(df1,df2,on='A')")
    st.write("Inner Join")
    st.code("pd.merge(df1, df2, on='A', how='left')")
    st.write("Left join")
    st.code("pd.merge(df1, df2, on='A', how='right')")
    st.write("Right join")
    st.code("pd.merge(df1, df2, on='A', how='outer')")
    st.write("Outer join")
    a.markdown("---")
#Data Manipulation Ends
#Time Starts
    a.title("10.Time Series Analysis")
